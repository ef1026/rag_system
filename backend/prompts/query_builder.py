...
def build_chat_query(
    ...
    hints = _format_retrieval_hints(retrieval_hints)
    if for_synthesis:
        return (
            f"用户原始问题：{question}\n"
            f"当前文档：{document_id}\n"
...
def _format_retrieval_hints(retrieval_hints: list[str] | None) -> str:
    hints = [hint.strip() for hint in retrieval_hints or [] if hint.strip()]
    if not hints:
        return ""
    lines = [
        "",
        "个性化召回提示（只影响找资料，不改变用户任务）：",
        *(f"- {hint}" for hint in hints[:6]),
    ]
    return "\n" + "\n".join(lines)
